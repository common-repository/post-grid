...
class PGBlockTeamMembersField
{
  ...
  {
    register_block_type(
      post_grid_plugin_dir . 'build/blocks/team-members-field/block.json',
      array(
  ...
  function theHTML($attributes, $content, $block)
  {
    ...
    // //* Visible condition
    $categories = isset($teamMemberData['categories']) ? $teamMemberData['categories'] : [];
    $teamSocialLinks = isset($teamMemberData['socialLinks']) ? $teamMemberData['socialLinks'] : [];
    ob_start();
?>
    <<?php echo pg_tag_escape($wrapperTag); ?> class="
      ...
      <?php if (!empty($fieldLink)) : ?>
        ...
        ?>
          <span class="fieldVal">
            <?php echo (isset($teamMemberData[$metaKey])) ? wp_kses_post($teamMemberData[$metaKey]) : '' ?>
          </span>
        <?php
        endif;
        ...
        ?>
          <span class="fieldVal social-links">
            ...
            foreach ($teamSocialLinks as $index => $item) :
              ...
              foreach ($socialLinks as $link) {
                if ($link['id'] === $item['id']) {
                  $socialLinkConfig = $link;
                  break;
                }
              }
              $socialIconHtml = '';
              // if (!empty($item['icon']['iconSrc'])) {
              $iconSrc = esc_attr($socialLinkConfig['siteIcon']['iconSrc']);
          ...
          if ($metaKey == 'image') :
          ?>
            <img class="fieldVal" src="<?php echo isset($teamMemberData['url']) ? esc_url($teamMemberData['url']) : ''; ?>"
              alt=" <?php echo isset($teamMemberData['title']) ? esc_attr($teamMemberData['title']) : ''; ?> " />
          <?php endif; ?>
          <?php
          ?>
...
$PGBlockTeamMembersField = new PGBlockTeamMembersField();

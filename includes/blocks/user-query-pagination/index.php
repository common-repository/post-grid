...
if (!defined('ABSPATH'))
  exit();
class PGBlockUserQueryPagination
{
  ...
  {
    register_block_type(
      post_grid_plugin_dir . 'build/blocks/user-query-pagination/block.json',
      array(
        'title' => "User Query - Pagination",
  ...
  {
    if (has_block('post-grid/user-query-pagination')) {
    ...
    $max_num_pages = isset($PGUserQuery->max_num_pages) ? $PGUserQuery->max_num_pages : 0;;
...
?>
    <?php if ($paginationType == 'normal') : ?>
      <div id="pagination-<?php echo esc_attr($blockId); ?>" class="pagination <?php echo esc_attr($blockId); ?> PGBlockPostGrid-pagination <?php echo esc_attr($paginationType); ?>" data-blockargs="<?php echo esc_attr(json_encode($blockArgs)); ?>">
        <?php
    ...
    <?php if ($paginationType == 'ajax') : ?>
      <div id="pagination-<?php echo esc_attr($blockId); ?>" class="pagination <?php echo esc_attr($blockId); ?> PGBlockPostGrid-pagination <?php echo esc_attr($paginationType); ?>" data-blockargs="<?php echo esc_attr(json_encode($blockArgs)); ?>">
        ...
        if (!empty($pages)) :
          foreach ($pages as $page) {
            //$links = str_replace('<a ', '<a data-blockargs="' . esc_attr(json_encode($blockArgs)) . '" ', $page);
            echo wp_kses_post($page);
          }
        endif;
        ?>
    ...
    <?php if ($paginationType == 'filterable') : ?>
      <div id="pagination-<?php echo esc_attr($blockId); ?>" class="<?php echo esc_attr($blockId); ?>  pagination PGBlockPostGrid-pagination <?php echo esc_attr($paginationType); ?> pager-list mixitup-page-list pager-list-<?php echo esc_attr($postGridId); ?>" data-blockargs="<?php echo esc_attr(json_encode($blockArgs)); ?>">
      </div>
    <?php endif; ?>
